...
class blendshape_controller(object):
	...
	def run_body_tracking(self):
		self.controller_dict['body_tracking_obj'] = body_tracking(self.terminate_process_shm, self.shm_cam, self.manager)
		self.controller_dict['body_tracking_obj'].run()
		
	def run_hand_tracking(self):
		self.controller_dict['hand_tracking_obj'] = hand_tracking(self.terminate_process_shm, self.shm_cam, self.manager)
		self.controller_dict['hand_tracking_obj'].run()

	def run_face_tracking(self):
		self.controller_dict['face_tracking_obj'] = face_tracking(self.terminate_process_shm, self.shm_cam, self.manager)
